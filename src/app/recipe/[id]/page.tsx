        ...
        <div className="min-h-screen mesh-gradient pb-20">
            ...
            <div className="max-w-3xl mx-auto px-6 -mt-32 relative z-10">
                ...
                >
                    <div className="flex justify-between items-start mb-6">
                        <div>
                            <div className="flex gap-2 mb-2">
                                {recipe.region && <span className="px-3 py-1 bg-white/10 rounded-full text-xs font-bold text-blue-300">{recipe.region}</span>}
                                {recipe.diet?.vegan && <span className="px-3 py-1 bg-green-500/20 rounded-full text-xs font-bold text-green-300">Vegan</span>}
                                {recipe.diet?.pescetarian && <span className="px-3 py-1 bg-blue-500/20 rounded-full text-xs font-bold text-blue-300">Pescetarian</span>}
                            </div>
                            ...
                            {recipe.macros && (
                                <div className="flex gap-4 text-sm">
                                    <div className="px-3 py-2 bg-white/5 rounded-lg border border-white/10">
                                        <span className="block text-xs text-gray-400 uppercase">Protein</span>
                                        <span className="font-bold text-white">{recipe.macros.protein}g</span>
                                    </div>
                                    <div className="px-3 py-2 bg-white/5 rounded-lg border border-white/10">
                                        <span className="block text-xs text-gray-400 uppercase">Carbs</span>
                                        <span className="font-bold text-white">{recipe.macros.carbs}g</span>
                                    </div>
                                    <div className="px-3 py-2 bg-white/5 rounded-lg border border-white/10">
                                        <span className="block text-xs text-gray-400 uppercase">Fat</span>
                                        <span className="font-bold text-white">{recipe.macros.fat}g</span>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    {(recipe.scienceNote || (recipe.macros && recipe.macros.protein > 0)) && (
                        <div className="mb-10 p-5 rounded-2xl bg-[var(--color-neon-lime)]/5 border border-[var(--color-neon-lime)]/30 backdrop-blur-sm">
                            ...
                            <p className="text-gray-300 text-sm leading-relaxed">
                                {recipe.scienceNote || `This meal is optimized for your vibe. It contains ${recipe.macros?.protein}g of protein, ${recipe.macros?.carbs}g of carbs, and ${recipe.macros?.fat}g of healthy fats to fuel your lifestyle.`}
                            </p>
                        </div>
                    )}
